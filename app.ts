...
app.get('/package/:id/rate', async (req : any, res : any) => {
  const authenticationToken = req.get('X-Authorization');
  await logger.info(`XAuth: ${authenticationToken}`)
  if(!authenticationToken || authenticationToken !== token) {
    return res.status(400).json('Auth not given');
...
app.get('/package/:packageId', async (req : any, res : any) => {
  const authenticationToken = req.get('X-Authorization');
  await logger.info(`XAuth: ${authenticationToken}`)
  if(!authenticationToken || authenticationToken !== token) {
    return res.status(400).json('Auth not given');
...
app.delete('/reset', async (req : any, res : any) => {
  ...
  if (authenticationToken === undefined) {
    return res.status(400).json('Auth doesnt exist');
  }
  if(!authenticationToken || authenticationToken !== token) {
...
app.get('/packageId/:packageName', async (req : any, res : any) => {
  ...
  try {
    ...
    const package_id = searchResults.map((data : any) => data.id);
...
app.delete('/package/:id', async (req : any, res : any) => {
  await logger.info("\n-----------------------------------------");
  await time.info("Starting time")
...
app.get('/package/byName/:name', async (req : any, res : any) => {
  await logger.info("\n-----------------------------------------");
...
app.delete('/package/byName/:name', async (req : any, res : any) => {
  await logger.info("\n-----------------------------------------");
