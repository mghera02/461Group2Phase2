...
import { Octokit } from "octokit"; // Octokit v17
...
import { get_github_info, url_list, get_npm_package_name, } from "./urls_parse";
const arg = process.argv[2]; // this is the url(s).txt arguement passed to the js executable
const npmPkgName = []; // setup array for package names
const gitDetails = []; // setup array for git user/repo name 
const gitUrls = []; // setup array for git urls
dotenv.config();
// Env variables
...
function runEslint(directory) {
    return new Promise((resolve, reject) => {
        exec(`npx eslint ${directory} -o ${directory}/result.json`, { encoding: 'utf8' }, (error, stdout, stderr) => {
            if (error) {
...
const readJSON = (jsonPath, callback) => {
...
function check_npm_for_open_source(filePath) {
    return new Promise((resolve) => {
        readJSON(filePath, (jsonData) => {
            if (jsonData !== null) {
                const repository = jsonData.repository;
    ...
    return __awaiter(this, void 0, void 0, function* () {
        for (let i = 0; i < pkgName.length; i++) {
            ...
            try {
                ...
                const gitURLfromNPM = yield check_npm_for_open_source(file);
                if (gitURLfromNPM) {
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (/\.txt$/.test(arg)) {
            ...
            yield get_npm_package_json(npmPkgName);
            try {
            ...
            yield get_metric_info(gitDetails);
...
export { octokit, logLevel, logFilePath, runEslint, ensureDirectoryExistence, check_npm_for_open_source, readJSON, };
